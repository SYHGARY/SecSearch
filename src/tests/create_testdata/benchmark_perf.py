...
使用方式:
    ...
    # 仅测试精确查询
    python tests/benchmark_perf.py --test exact

    # 指定测试样本数
    python tests/benchmark_perf.py --samples 100

    # 批量解密并发线程数
    python tests/benchmark_perf.py --decrypt-threads 8
"""
...
def benchmark_concurrent_consistency(num_threads: int = 8, per_thread: int = 100) -> dict:
    ...
    return {
        ...
        "tag_verify_pass": f"{tag_ok}/{len(samples)}",
        "errors": errors,
        "pass": actual_count == total and len(errors) == 0 and tag_ok == len(samples),
    }
